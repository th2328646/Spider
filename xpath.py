...
def spider(in_url, in_href, classify):
    ...
    url_seen = []
    for url_article in url_list[0:1]:
        if url_article in url_seen:
            ...
            html_all = get.read()
            tree = etree.HTML(html_all)
            # 获取文章标题 article_title
            article_title = tree.xpath('//div[@class = "art_box"]/h1/text()')
            # print article_title[0].encode('raw-unicode-escape')

            # 获取文章核心提示 article_tips
            article_tips = tree.xpath('//p[@class = "summary"]//text()')
            # print article_tips[0].encode('raw-unicode-escape')

            # 获取文章正文内容 article_content
            article_content1 = tree.xpath('//div[@class = "art_con"]/descendant::*')
            for article_content2 in article_content1:
                article_content3 = article_content2.xpath('//div[@class = "hzh_botleft"]/preceding-sibling::*')
                for article_content in article_content3:
                    print article_content
                    a = article_content.xpath('//p|//strong|//span')
                    for b in a:
                        c = b.xpath('text()')
                        for d in c:
                            print d.encode('raw-unicode-escape')
#             content_list = []
